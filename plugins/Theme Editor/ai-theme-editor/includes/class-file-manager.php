...
class WPAI_File_Manager {
    ...
    public function resolve_path( string $file_key ) {
        ...
        if ( $resolved_root === false
             || ( strpos( $candidate, $resolved_root . DIRECTORY_SEPARATOR ) !== 0 && $candidate !== $resolved_root ) ) {
    ...
    public function read_file( string $file_key ) {
        ...
        $content = $wp_filesystem ? $wp_filesystem->get_contents( $path ) : @file_get_contents( $path );
    ...
    /**
     * Writes content to a file (no backup — caller must create backup first).
     *
     * @param string $file_key
     ...
     * @param string $filename    Relative path e.g. "page-about.php"
     * @param string $content     File content to write
     * @return string|WP_Error    File key on success e.g. "theme:page-about.php"
     */
    public function create_file( string $destination, string $filename, string $content ) {
        if ( $destination === 'theme' ) {
            ...
            if ( realpath( $root ) === realpath( get_template_directory() ) ) {
                return new WP_Error( 'no_child_theme', 'No child theme is currently active.', array( 'status' => 400 ) );
            }
        } else {
        ...
        if ( file_exists( $full_path ) ) {
            return new WP_Error( 'file_exists', 'A file with that name already exists.', array( 'status' => 409 ) );
        }
        ...
        $ok = $wp_filesystem
            ? $wp_filesystem->put_contents( $full_path, $content, FS_CHMOD_FILE )
    ...
    public function create_child_theme( string $theme_name, array $files ) {
        ...
        foreach ( $files as $filename => $file_content ) {
            ...
            if ( $wp_filesystem ) {
                $wp_filesystem->put_contents( $filepath, $file_content, FS_CHMOD_FILE );
            } else {
    ...
    private function scan_directory(
        ...
        foreach ( $items as $item ) {
            ...
            if ( is_dir( $full_path ) ) {
                $files = array_merge( $files, $this->scan_directory( $full_path, $extensions, $group, $key_prefix, $root, $depth + 1 ) );
