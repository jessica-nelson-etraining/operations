 ...
 *
 * NOTE: Edit mode uses the streaming endpoint (WPAI_Stream_Handler / wp_ajax_wpai_stream)
 * rather than /ask, to avoid PHP/HTTP timeouts on large files.
 */
...
class WPAI_Rest_API {
    ...
    /**
     * POST /files — Returns the list of editable files for the dropdown.
     */
    public function handle_get_files( WP_REST_Request $request ): WP_REST_Response {
        $file_manager = new WPAI_File_Manager();
        return new WP_REST_Response( array( 'files' => $file_manager->get_file_list() ), 200 );
    }
    ...
    /**
     * POST /ask — Question mode only. Edit mode uses the streaming endpoint.
     *
     * Body params:
     *   file_key  string
     *   message   string
     *   mode      string  'question' (edit mode calls wp_ajax_wpai_stream instead)
     */
    public function handle_ask( WP_REST_Request $request ): WP_REST_Response {
        ...
        if ( $mode === 'edit' ) {
            // Fallback — normally edit goes through the streaming endpoint
            ...
            if ( $original_len > 500 && $modified_len < $original_len * 0.8 ) {
                return $this->error( 'The AI returned an incomplete file. Please try again with a simpler instruction.', 500 );
            }
    ...
    public function handle_apply( WP_REST_Request $request ): WP_REST_Response {
        ...
        if ( empty( $file_key ) ) return $this->error( 'file_key is required', 400 );
        if ( $content === '' )    return $this->error( 'content cannot be empty', 400 );
        ...
        if ( is_wp_error( $write ) ) {
            return $this->error( 'Write failed: ' . $write->get_error_message(), 500 );
        }
    ...
    public function handle_list_backups( WP_REST_Request $request ): WP_REST_Response {
        ...
        $backup_manager = new WPAI_Backup_Manager();
        return new WP_REST_Response( array( 'backups' => $backup_manager->list_backups( $file_key ) ), 200 );
    }
    ...
    public function handle_preview( WP_REST_Request $request ): WP_REST_Response {
        ...
        if ( is_wp_error( $preview_url ) ) return $this->from_wp_error( $preview_url );
     ...
     *
     * Body params:
     *   type         'file' or 'child-theme'
     *   filename     For type=file
     *   description  What the file/theme should do
     *   theme_name   Active or parent theme name
     *   parent_slug  For type=child-theme
     *   child_name   For type=child-theme
     */
    public function handle_generate_content( WP_REST_Request $request ): WP_REST_Response {
        ...
        if ( empty( $api_key ) ) {
            return $this->error( 'Anthropic API key not configured.', 400 );
        }
     ...
     *
     * Body params:
     *   destination  'theme' or 'child'
     *   filename     Relative filename e.g. "page-about.php"
    ...
    public function handle_create_file( WP_REST_Request $request ): WP_REST_Response {
        ...
        if ( empty( $filename ) ) return $this->error( 'filename is required', 400 );
     ...
     *
     * Body params:
     *   theme_name  Human-readable child theme name
     *   files       { 'style.css': content, 'functions.php': content, 'index.php': content }
     */
    public function handle_create_child_theme( WP_REST_Request $request ): WP_REST_Response {
        ...
        if ( empty( $theme_name ) ) return $this->error( 'theme_name is required', 400 );
        if ( ! is_array( $files ) || empty( $files ) ) return $this->error( 'files must be a non-empty object', 400 );
